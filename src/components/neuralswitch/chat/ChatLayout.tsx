...
import { api, streamChat } from "@/lib/neuralswitch/api";
import type {
  AttachmentItem,
  ChatMessage,
  ChatMode,
  ...
  Source,
  StreamEvent,
} from "@/lib/neuralswitch/types";
import { ChatSidebar } from "./ChatSidebar";
import { ChatInput } from "./ChatInput";
import { MessageList } from "./MessageList";
import { ChatTopBar } from "./ChatTopBar";
import { SettingsModal } from "@/components/neuralswitch/settings/SettingsModal";

const SUGGESTIONS = [
  "Which BDM has the highest budget?",
  "Show actual revenue by customer for Q1",
  "What is the latest market news today?",
  "Compare forecast versus budget for MS",
];
...
export function ChatLayout() {
  ...
  const streamRunIdRef = React.useRef(0);
  ...
  const openChat = async (id: string) => {
    if (isGenerating) cancelActiveStream();
    setError(null);
    setActiveChatId(id);
    setSuggested([]);
    try {
  ...
  const runStream = (text: string, baseMessages: ChatMessage[]) => {
    streamRunIdRef.current += 1;
    const streamId = streamRunIdRef.current;
    setError(null);
    setSuggested([]);
    setToolStatus("");
    ...
    controllerRef.current = streamChat(
      ...
      (e: StreamEvent) => {
        if (streamId !== streamRunIdRef.current) return;
        if (e.type === "meta") {
          ...
          setMessages((prev) => {
            ...
            if (last && last.role === "assistant") {
              const nextMetadata = {
                ...
                ...(e.chart ? { chart: e.chart } : {}),
              };
              next[next.length - 1] = {
                ...last,
                id: e.message_id || "done",
                pending: false,
                content: e.answer ?? last.content,
