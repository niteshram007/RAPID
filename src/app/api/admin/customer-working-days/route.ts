...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(
    `${BACKEND_API_URL}/api/admin/customer-working-days?${request.nextUrl.searchParams.toString()}`,
...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${BACKEND_API_URL}/api/admin/customer-working-days`, {
    method: "POST",
