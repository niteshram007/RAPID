...
import { submitForecastSheet } from "@/lib/rapid-revenue-server";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await submitForecastSheet(session, payload.forecastMonth, payload.rows);
    return NextResponse.json(response);
  } catch (error) {
    return NextResponse.json(
      { detail: error instanceof Error ? error.message : "Unable to save forecast." },
      { status: 400 },
    );
  }
}
