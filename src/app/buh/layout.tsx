...
export default async function BuhLayout({ children }: { children: React.ReactNode }) {
  const session = await requirePermission("view_dashboard");

  if (!canAccessWorkspaceArea(session.role, "buh")) {
  ...
  return (
    <WorkspaceShell
      ...
      title=""
      description=""
      navItems={buildWorkspaceNav(session.role, "/buh")}
    >
      {children}
    </WorkspaceShell>
  );
